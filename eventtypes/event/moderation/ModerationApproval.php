...
class ModerationApproval
{
    ...
    public static function fetchList(int $limit, int $offset = 0, $filters = [], $groupByObjectId = true): array
    {
        ...
        $query .= "select ezcollab_item.*, ezapprove_items.workflow_process_id from ezcollab_item 
    ...
    private static function parseFiltersToSqlConditions(array $filters): string
    {
        ...
        if (!empty($filters['classIdentifier'])) {
            $conditions[] = "ezcollab_item.data_text1 = '" . eZDB::instance()->escapeString($filters['classIdentifier']) . "'";
        }
    ...
    public static function fetchPendingCountByObjectIdAndLocale(int $objectId, string $locale): int
    {
        ...
        $tasks = eZDB::instance()->arrayQuery(
            "select ezcollab_item.*, ezapprove_items.workflow_process_id from ezcollab_item 
    ...
    public static function store(ModerationApproval $moderationApproval)
    {
        eZDB::instance()->begin();
        ...
        $collaborationItem->setAttribute('data_float1', (int)$moderationApproval->createdAt);
    ...
    public static function updateStatus(ModerationApproval $moderationApproval)
    {
        ...
        if ($moderationApproval->status === ModerationHandler::STATUS_REFUSED) {
            ...
            if ($contentVersion) {
                $contentVersion->setAttribute('status', eZContentObjectVersion::STATUS_REJECTED);
                $contentVersion->sync();
            }
        }
    }
    ...
    private static function serialize(array $item): ModerationApproval
    {
        ...
        $instance->createdAt = (int)$item['data_float1'];
        ...
        if (!isset($this->attributes[$key])) {
            switch ($key) {
                ...
                case 'can_edit':
                    ...
                    break;
                case 'version':
                    $this->attributes[$key] = eZContentObjectVersion::fetch($this->contentObjectVersionId);
                    break;
                default:
