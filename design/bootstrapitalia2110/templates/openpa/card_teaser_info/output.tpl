...
{def $show_link = cond($attributes.show|contains('content_show_read_more'), true(), false())}
{if and($node|has_attribute('abstract')|not(), $node|has_attribute('description')|not())}
    {set $show_link = false()}
{/if}
<div data-object_id="{$node.contentobject_id}" class="font-sans-serif card card-teaser card-teaser-info rounded shadow-sm p-3 {if $view_variation|eq('auto_width')|not()}card-teaser-info-width {/if} mt-0 mb-3 {$view_variation}" style="z-index: {100|sub($attribute_index)}">
    {if $has_image}
    <div class="card-image-wrapper" style="flex: 1;display: flex;flex-direction: row;justify-content: space-between;align-items: flex-start;">
    {/if}
    <div class="card-body {if $has_image}pe-3{/if}">
        ...
        {if and($hide_title, $show_link, $node.object.state_identifier_array|contains('privacy/public'))}
