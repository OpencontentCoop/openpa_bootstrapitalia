...
<div class="container-fluid my-3">
    <div class="row">
        <figure class="figure px-0{if and(is_set($openpa.event_link.image.width), is_set($openpa.event_link.image.height), image_class_and_style($openpa.event_link.image.width, $openpa.event_link.image.height).can_enlarge)}img-full {/if} d-block text-center" xmlns="http://www.w3.org/1999/html">
            <img class="{if and(is_set($openpa.event_link.image.width), is_set($openpa.event_link.image.height))}{image_class_and_style($openpa.event_link.image.width, $openpa.event_link.image.height).css_class}{/if}"
                 style="{if and(is_set($openpa.event_link.image.width), is_set($openpa.event_link.image.height))}{image_class_and_style($openpa.event_link.image.width, $openpa.event_link.image.height).inline_style}{/if}"
                 src="{$openpa.event_link.image.url}"
                 alt="{$node.name|wash()}"
                 loading="lazy" />
            {if or(and(is_set($openpa.event_link.image.author), $openpa.event_link.image.author), and(is_set($openpa.event_link.image.license), $openpa.event_link.image.license))}
...
<section class="container">
    ...
    {if $node|has_attribute('about_target_audience')}
        {set $show = $show|append('a_chi_e_rivolto')}
    {/if}
    {if or($openpa.event_link.takes_place_in, $openpa.event_link.geo)}
        {set $show = $show|append('luogo')}
    {/if}
    {if or($node|has_attribute('cost_notes'), $openpa.event_link.has_offer|count())}
        {set $show = $show|append('costi')}
    {/if}
    {if or($openpa.event_link.has_online_contact_point, $openpa.event_link.has_online_contact_info)}
        {set $show = $show|append('contatti')}
    {/if}
