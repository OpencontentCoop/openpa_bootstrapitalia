...
*}{foreach $main_attributes as $identifier}{*
	*}{if $node|has_attribute($identifier)}{*
		*}{if $node|attribute($identifier).data_type_string|eq('ezimage')}{*
			...
			*}{$image.content[$image_class].url|ezroot(no,full)|explode('http://')|implode('https://')}{undef $image}{*
		*}{elseif $node|attribute($identifier).data_type_string|eq('ezobjectrelationlist')}{*
			*}{foreach $node|attribute($identifier).content.relation_list as $item}{*
				*}{include name="related_main_image_as_background" uri='design:newsletter/skin/bootstrapitalia/outputformat/include/image_url.tpl' node=fetch(content,node, hash(node_id, $item.node_id))}{*
				*}{break}{*
			*}{/foreach}{*
		*}{/if}{*
	*}{break}{*
