...
{if and(is_set($#context_object_id), $#context_object_id)}
	{set $context = hash('contentobject_id', $#context_object_id|int())}
{/if}
			...
			{if and($attribute_group.slug|eq('details'), count($current_entities)|gt(0))}
				<div class="card-wrapper card-teaser-wrapper card-teaser-wrapper-equal card-teaser-block-2" style="min-width:49%">
					{foreach $current_entities as $child }
						{node_view_gui content_node=$child.main_node view=$view show_icon=false() show_category=false() image_class=widemedium}
					{/foreach}
				</div>
			{/if}
