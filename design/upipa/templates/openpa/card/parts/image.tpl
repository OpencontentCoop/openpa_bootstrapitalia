{if $has_media}
    <div class="img-responsive-wrapper">
        <div class="img-responsive{if $view_variation|eq('big')} img-responsive-panoramic{/if}">
            ...
            {elseif $node|has_attribute('from_time')}
                <div class="card-calendar d-flex flex-column justify-content-center">
                    <span class="card-date">{$node|attribute('from_time').data_int|datetime( 'custom', '%j' )}</span>
                    <span class="card-day">{$node|attribute('from_time').data_int|datetime( 'custom', '%F' )}</span>
                </div>
            {/if}
        </div>        
    </div>
