...
{foreach $matrix.columns.sequential as $index => $column}
	...
	{/foreach}
	{if count($value)|gt(0)}
		<dt class="col-2 text-nowrap">{$column.identifier|i18n('bootstrapitalia/opening_hours_matrix_short')|wash()}</dt>
		<dd class="col-10"><span class="text-nowrap">{$value|implode('</span>, <span class="text-nowrap">')}</span></dd>
		{set $i = $i|inc()}
	{/if}
	{undef $value}
