...
{if $attribute.data_text}
    <a class="{$css_class}"
      ...
      {$attribute.data_text|wash( xhtml )}{if $is_channel|not()} <i aria-hidden="true" class="fa fa-external-link"></i>{/if}
      </a>
{else}
    <a class="{$css_class}"
      {if $is_channel|not()}target="_blank" rel="noopener noreferrer"{/if}
      href="{$attribute.content|wash( xhtml )}">
      {$attribute.content|wash( xhtml )}{if $is_channel|not()} <i aria-hidden="true" class="fa fa-external-link"></i>{/if}
    </a>
{/if}
