...
{if count($node_list)|gt(0)}
    {foreach $node_list as $index => $child}
        ...
        {if $child.data_map.channel_url.has_content}
            <div class="mb-4">
                {attribute_view_gui attribute=$child|attribute('channel_url') css_class=cond($index|eq(0), "btn btn-success fw-bold mobile-full font-sans-serif", "btn btn-outline-primary t-primary bg-white mobile-full font-sans-serif")}
            </div>
        {/if}
    {/foreach}
{/if}
