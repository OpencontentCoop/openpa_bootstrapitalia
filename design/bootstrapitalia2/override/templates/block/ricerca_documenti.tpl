			...
			<div class="col-12 col-lg-4 ps-lg-5 order-first order-md-last">
				...
				<div class="accordion">
					...
					{if and($filters|contains('area'), $area_count|gt(-1))}
						<div class="accordion-item bg-none">
						  <span class="accordion-header" id="collapseArea-{$block.id}-title">
							<button class="accordion-button collapsed pb-10 px-3 text-uppercase text-decoration-none" type="button"
									data-bs-toggle="collapse" href="#collapseArea-{$block.id}" role="button" aria-expanded="false" aria-controls="collapseArea-{$block.id}"
									data-focus-mouse="false">
								{'Administrative area'|i18n('bootstrapitalia/documents')}
							</button>
						  </span>
							<div id="collapseArea-{$block.id}" class="accordion-collapse collapse" role="region" aria-labelledby="collapseArea-{$block.id}-title">
								<div class="accordion-body">
									<label for="searchFormArea-{$block.id}" class="visually-hidden"><small>{'Administrative area'|i18n('bootstrapitalia/documents')}</small></label>
									<select class="form-control border-bottom" id="searchFormArea-{$block.id}" data-search="area">
										<option value=""></option>
			...
			<div class="card shadow-sm px-4 pt-4 pb-4 rounded">
				...
				<div class="card-body p-0 my-2">
					<h3 class="green-title-big t-primary mb-8">
						<a class="text-decoration-none" href="{{:baseUrl}}content/view/full/{{:metadata.mainNodeId}}">{{:~i18n(metadata.name)}}</a>
					</h3>
