...
{if ezini_hasvariable(concat('LoginTemplate_', $login_module_setting), 'Text', 'app.ini')}
  <p class="text-center" style="margin-bottom: 20px">
    {ezini(concat('LoginTemplate_', $login_module_setting), 'Text', 'app.ini')|wash()}</p>
{/if}
...
<form style="max-width: 400px;margin: 0 auto;" class="validate-form border px-4 pt-5 pb-4 rounded bg-white text-center"
  method="post" action='{"/user/login/"|ezurl(no)}' name="loginform">
  ...
  <div class='form-group'>
    <div class='controls with-icon-over-input'>
      <input type="password" autocomplete="off" name="Password"
        placeholder="{"Password"|i18n("design/ocbootstrap/user/login")}" class="form-control no-copy"
        data-rule-required="true">
      <i class='icon-lock text-muted'></i>
    </div>
  </div>
  <button class='btn btn-lg btn-primary center-block' name="LoginButton">
