            ...
            {if and($node|has_attribute('show_topic_children'), $node|attribute('show_topic_children').data_int|eq(1))}
                <div class="mt-4">
                    {foreach $node.children as $child}
                        ...
                        <a class="text-decoration-none text-nowrap d-inline-block mb-1" href="{$child.url_alias|ezurl(no)}"
                           data-eurovoc="{$child|attribute('eu').data_text|wash()}" data-element="{object_handler($child).data_element.value|wash()}">
                            <div class="chip chip-simple chip-primary {if $child.object.section_id|ne(1)}no-sezioni_per_tutti{/if}"><span class="chip-label">{$child.name|wash()}</span></div>
                        </a>
                        {/if}
                    {/foreach}
                </div>
