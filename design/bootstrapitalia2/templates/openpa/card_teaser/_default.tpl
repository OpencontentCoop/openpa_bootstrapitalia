...
<div data-object_id="{$node.contentobject_id}"
    ...
    {if or($attributes.show|contains('content_show_read_more'), $attributes.enabled|not())}
        <a data-element="{$openpa.data_element.value|wash()}" class="read-more{if $has_image} ps-3 position-absolute bottom-0 mb-3{/if}" href="{$openpa.content_link.full_link}" aria-label="{'Go to content'|i18n('bootstrapitalia')} {$node.name|wash()}">
