...
{foreach $main_attributes as $identifier}
	{if and(or($identifier|eq('alternative_name'), $identifier|eq('alt_name')), is_set($openpa[$identifier].contentobject_attribute))}
		<p class="subtitle-small mb-3" data-element="service-description">{attribute_view_gui attribute=$openpa[$identifier].contentobject_attribute image_class=reference alignment=center}</p>
	{/if}
