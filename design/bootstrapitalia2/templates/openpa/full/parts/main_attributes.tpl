...
{foreach $main_attributes as $identifier}
	{if and(or($identifier|eq('alternative_name'), $identifier|eq('alt_name')), is_set($openpa[$identifier].contentobject_attribute))}
		<p class="subtitle-small mb-3"{if $node.object.class_identifier|eq('public_service')} data-element="service-description"{/if}>{attribute_view_gui attribute=$openpa[$identifier].contentobject_attribute image_class=reference alignment=center}</p>
	{/if}
...
{if or($main_attributes|contains('content_show_published'),$main_attributes|contains('content_show_modified'),$main_attributes|contains('reading_time'),count($datetime_attributes)|gt(0))}
	...
	{if and($node|has_attribute('reading_time'), $main_attributes|contains('reading_time'), $node|attribute('reading_time').content|ne('0'))}
		<div class="col">
			<small class="d-block text-nowrap font-sans-serif">{'Reading time'|i18n('bootstrapitalia')}:</small>
			<p class="fw-semibold font-monospace text-nowrap">{$node|attribute('reading_time').content|wash()} min</p>
		</div>
	{/if}
	{foreach $datetime_attributes as $attribute}
		<div class="col">
			<small class="d-block text-nowrap font-sans-serif">{$attribute.contentclass_attribute_name|wash()}:</small>
			<p class="fw-semibold font-monospace text-nowrap">{$attribute.content.timestamp|l10n( 'date' )}</p>
		</div>
	{/foreach}
	</div>
{/if}
