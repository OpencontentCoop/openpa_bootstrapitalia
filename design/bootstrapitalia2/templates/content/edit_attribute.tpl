    ...
    {if $edit_attribute_groups.count|gt(1)}
        <div class="col-3 affix-parent">
            <ul id="edit-groups" class="affix-top nav nav-tabs nav-tabs-vertical" role="tablist" aria-orientation="vertical">
                <li class="ps-0 pt-4 pb-2 text-uppercase"><span>{'Table of contents'|i18n('bootstrapitalia')}</span></li>
                {foreach $edit_attribute_groups.groups as $index => $attribute_group}
                    {if $attribute_group.show}
                        <li class="nav-item">
                            <a class="w-100 nav-link ps-0{if $index|eq(0)} active{/if}" data-bs-toggle="tab" data-toggle="tab" data-bs-target="#attribute-group-{$attribute_group.identifier}"
                                ...
                                <div class="col-12">
                                    ...
                                    <div>
                                        {if $attribute.display_info.edit.grouped_input}
                                            <fieldset>
                                                {if and(ezini('BandoCigSettings', 'AnacImport', 'openpa.ini')|eq('enabled'), ezini('BandoCigSettings', 'AnacImportedFields', 'openpa.ini')|contains($attribute_identifier))}<p class="text-muted small mt-1"><em>{'Anac attribute update alert'|i18n('bootstrapitalia')}</em></p>{/if}{if and(ezini('BandoCigSettings', 'AnacImport', 'openpa.ini')|eq('enabled'), ezini('BandoCigSettings', 'CigField', 'openpa.ini')|contains($attribute_identifier))}<p class="text-muted small mt-1"><em>{'Anac cig attribute alert'|i18n('bootstrapitalia')}</em></p>{/if}
