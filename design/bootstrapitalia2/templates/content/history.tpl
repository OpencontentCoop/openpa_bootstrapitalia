    ...
    <form name="versionsform" action="{concat( '/content/history/', $object.id, '/' )|ezurl(no)}" method="post">
        ...
        {if $list_count}
            ...
            <tr {if array(2)|contains($version.status)}class="bg-light"{/if}>
                ...
                {* Approvw button. *}
                <td style="vertical-align:middle">
                    {if array(2)|contains($version.status)}
                        {if and( can_approve_version($version.id), $can_edit ) }
                            <div style="white-space:nowrap">
                                <a class="px-2" href="{concat('/bootstrapitalia/approval/', $version.id, '/approve?redirect=history')|ezurl(no)}" title="{'Approve'|i18n('design/standard/collaboration/approval')}">
                                    <span class="fa-stack">
                                      <i class="fa fa-circle fa-stack-2x text-success"></i>
                                      <i class="fa fa-thumbs-o-up fa-stack-1x fa-inverse"></i>
                                    </span>
                                </a>
                                <a class="px-2" href="{concat('/bootstrapitalia/approval/', $version.id, '/deny?redirect=history')|ezurl(no)}" title="{'Deny'|i18n('design/standard/collaboration/approval')}">
                                    <span class="fa-stack">
                        ...
                        {else}
                            <small style="min-width:180px;display:block;line-height: 1.3;">
                                {"The content object %1 awaits approval before it can be published."|i18n('design/standard/collaboration/approval',,array(''))}
                            </small>
                        {/if}
                    {/if}
                </td>
