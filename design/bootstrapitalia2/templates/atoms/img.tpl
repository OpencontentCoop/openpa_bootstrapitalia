...
{if is_set($node)}
<img {if openpaini('InstanceSettings', 'LazyLoadImages', 'disabled')|eq('enabled')}src="{'images/placeholder.gif'|ezdesign(no)}" data-{/if}src="{node_image($node,$image_class).full_path|ezroot(no)|preload_image()}"
