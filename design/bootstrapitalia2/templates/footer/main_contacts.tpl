                            ...
                            <ul class="contact-list p-0">
                                ...
                                <li>
                                    <a class="list-item" href="{object_handler($faq_system).content_link.full_link}" data-element="faq">
                                        {display_icon('it-help-circle', 'svg', 'icon icon-primary', 'Read the FAQ'|i18n('bootstrapitalia'))}<span>{'Read the FAQ'|i18n('bootstrapitalia')}</span>
                                    </a>
                                </li>
                                {/if}
                                {undef $faq_system}
                                ...
                                <li>
                                    <a class="list-item" href="{if is_set($pagedata.contacts['link_assistenza'])}{$pagedata.contacts['link_assistenza']|wash()}{else}{'richiedi_assistenza'|ezurl(no)}{/if}" data-element="contacts">
                                        {display_icon('it-mail', 'svg', 'icon icon-primary', 'Request assistance'|i18n('bootstrapitalia'))}<span>{'Request assistance'|i18n('bootstrapitalia')}</span>
                                    </a>
                                </li>
                                {/if}
                                ...
                                <li>
                                    <a class="list-item" href="{if is_set($pagedata.contacts['numero_verde'])}tel:{$pagedata.contacts['numero_verde']|wash()}{else}tel:{$pagedata.contacts['telefono']|wash()}{/if}">
                                        {display_icon('it-hearing', 'svg', 'icon icon-primary', 'Call the municipality'|i18n('bootstrapitalia'))}<span>{'Call the municipality'|i18n('bootstrapitalia')} {if is_set($pagedata.contacts['numero_verde'])}{$pagedata.contacts['numero_verde']|wash()}{else}{$pagedata.contacts['telefono']|wash()}{/if}</span>
                                    </a>
                                </li>
                                {/if}
                                ...
                                <li>
                                    <a class="list-item" href="{if is_set($pagedata.contacts['link_prenotazione_appuntamento'])}{$pagedata.contacts['link_prenotazione_appuntamento']|wash()}{else}{'prenota_appuntamento'|ezurl(no)}{/if}" data-element="appointment-booking">
                                        {display_icon('it-calendar', 'svg', 'icon icon-primary', 'Book an appointment'|i18n('bootstrapitalia'))}<span>{'Book an appointment'|i18n('bootstrapitalia')}</span>
                                    </a>
                                </li>
                                {/if}
                                {undef $context}
                            ...
                            <ul class="contact-list p-0">
                                <li>
                                    <a class="list-item" data-element="report-inefficiency" href="{if is_set($pagedata.contacts['link_segnalazione_disservizio'])}{$pagedata.contacts['link_segnalazione_disservizio']|wash()}{else}{'segnala_disservizio'|ezurl(no)}{/if}">
                                        {display_icon('it-map-marker-circle', 'svg', 'icon icon-primary', 'Report a inefficiency'|i18n('bootstrapitalia'))}<span>{'Report a inefficiency'|i18n('bootstrapitalia')}</span>
                                    </a>
                                </li>
                            </ul>
                            {/if}
