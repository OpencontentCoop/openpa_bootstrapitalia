                            ...
                            <ul class="contact-list p-0">
                                ...
                                <li>
                                    <a class="list-item" href="{if is_set($pagedata.contacts['numero_verde'])}tel:{$pagedata.contacts['numero_verde']|wash()}{else}tel:{$pagedata.contacts['telefono']|wash()}{/if}">
                                        {display_icon('it-hearing', 'svg', 'icon icon-primary')}<span>{'Call the municipality'|i18n('bootstrapitalia')} {if is_set($pagedata.contacts['numero_verde'])}{$pagedata.contacts['numero_verde']|wash()}{else}{$pagedata.contacts['telefono']|wash()}{/if}</span>
                                    </a>
                                </li>
                                {/if}
