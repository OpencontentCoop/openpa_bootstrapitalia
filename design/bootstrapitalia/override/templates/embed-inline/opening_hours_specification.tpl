        ...
        {if $object|has_attribute($identifier)}
            ...
            {if $object|attribute($identifier).data_type_string|eq('ezobjectrelationlist')}
                {foreach $object|attribute($identifier).content.relation_list as $item}
                    ...
                    <p>{content_view_gui content_object=$item_object view=embed-inline show_icon=false() show_link=cond($attributes.show_link|contains($identifier), true(), false())}</p>
                    {undef $item_object}
                {/foreach}
            {elseif $object|attribute($identifier).data_type_string|ne('ezimage')}
                {attribute_view_gui attribute=$object|attribute($identifier) image_class=small}
            {/if}
            </div>
        {/if}        
    {/foreach}
