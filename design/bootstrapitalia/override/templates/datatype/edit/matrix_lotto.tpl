    ...
    <table class="table table-sm">
        ...
        <tr>
            ...
            {section var=ColumnNames loop=$matrix.columns.sequential}
                <th class="size-sm text-nowrap">{$ColumnNames.item.name}</small></th>
            {/section}
        </tr>
                ...
                {section var=Columns loop=$Rows.item.columns}
                    <td>
                        {if $Columns.index|eq(4)}
                            <select id="ezcoa-{if ne( $attribute_base, 'ContentObjectAttribute' )}{$attribute_base}-{/if}{$attribute.contentclassattribute_id}_{$attribute.contentclass_attribute_identifier}_matrix_cell_{$Rows.index}_{$Columns.index}"
                                   class="form-control p-0 border-bottom border-black ezcc-{$attribute.object.content_class.identifier} ezcca-{$attribute.object.content_class.identifier}_{$attribute.contentclass_attribute_identifier}"
                                   name="{$attribute_base}_ezmatrix_cell_{$attribute.id}[]">
                                <option value=""{if $Columns.item|eq('')} selected="selected"{/if}></option>
                                <option value="1"{if $Columns.item|eq('1')} selected="selected"{/if}>Mandante</option>
                                <option value="2"{if $Columns.item|eq('2')} selected="selected"{/if}>Mandataria</option>
                                <option value="3"{if $Columns.item|eq('3')} selected="selected"{/if}>Associata</option>
                                <option value="4"{if $Columns.item|eq('4')} selected="selected"{/if}>Capogruppo</option>
                                <option value="5"{if $Columns.item|eq('5')} selected="selected"{/if}>Consorziata</option>
                            </select>
                        {else}
                        <input id="ezcoa-{if ne( $attribute_base, 'ContentObjectAttribute' )}{$attribute_base}-{/if}{$attribute.contentclassattribute_id}_{$attribute.contentclass_attribute_identifier}_matrix_cell_{$Rows.index}_{$Columns.index}"
                               class="form-control form-control-sm ezcc-{$attribute.object.content_class.identifier} ezcca-{$attribute.object.content_class.identifier}_{$attribute.contentclass_attribute_identifier}"
