...
<div class="my-3">
    ...
    <dl class="row {if $index|gt(0)} border-top pt-3{/if}">
        ...
        <dd class="col-4 col-sm-3 text-center">
            <span style="font-size: 1em" class="badge badge-light font-weight-normal ml-2 ms-2">{if or($row.columns[2]|eq(''), $row.columns[2]|eq('0'))}{'FREE'|i18n('bootstrapitalia')}{else}{$row.columns[2]|wash()} {$row.columns[3]|wash()}{/if}</span>
        </dd>
        {else}
        <dd class="col">
