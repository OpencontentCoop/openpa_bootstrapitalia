;(function ($, window, document, undefined) {
    ...
    function Plugin(element, options) {
        ...
        this.toggleSearch = $('#toggleDashboardSearch');
        ...
        this.searchQuery = $('.dashboard-search-query');
        ...
        $.views.helpers($.extend({}, $.opendataTools.helpers, {
            ...
            'itemMainImageUrl': function (data) {
                var images = $.opendataTools.helpers.i18n(data, 'image');
                if (images.length > 0) {
                    return $.opendataTools.settings('accessPath') + '/openpa/image/' + images[0].id;
                }
                return null;
            },
        ...
        this.stateToggles = $('a.dashboard-state-filter');

        this.spinnerTpl = $($.templates('#tpl-spinner').render({}));
        this.listTpl = $.templates('#tpl-dashboard-list');
        this.currentPage = 0;
        this.queryPerPage = [];
        this.currentView = $('a.dashboard-view-selector.active').attr('href');
    ...
    $.extend(Plugin.prototype, {
        ...
        addListeners: function () {
            ...
            var stateFilters = $("ul.state-filters a.dashboard-state-filter");
            ...
            $('body').on('shown.bs.tab', function (e) {
                if ($(e.target).hasClass('dashboard-view-selector')) {
        ...
        refreshAllChipToggle: function () {
            ...
            if ($('.dashboard-search-form-filters a:visible').length === chipLength && !hasQueryInput) {
        ...
        runSearch: function () {
            ...
            if (plugin.currentView === '#list' && plugin.needRefresh.list) {
                plugin.renderList(plugin.listTpl);
                plugin.needRefresh.list = false;
        ...
        renderList: function (template) {
            var plugin = this;
            var resultsContainer = $('.items ' + plugin.currentView);
