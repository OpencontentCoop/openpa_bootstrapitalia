    ...
    $('[data-shared_link]').each(function () {
        ...
        function loadRemoteView(objectId){
            ...
            $.ajax({
                ...
                url: remoteApi,
                dataType: 'jsonp',
                jsonpCallback: function() { // allow remote cache
                  let cb = 'shared_link_'+objectId;
                  // avoid callback duplication
                  while (sharedLinkCallbacks.includes(cb)) {
                    cb += '_'
                  }
                  sharedLinkCallbacks.push(cb)
                  return cb
                },
