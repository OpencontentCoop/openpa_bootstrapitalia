{* DO NOT EDIT THIS FILE! Use an override template instead. *}
{default attribute_base=ContentObjectAttribute html_class='form-control'}
{def $is_valuation = cond($attribute.object.class_identifier|eq('valuation'), true(), false())}
{let selected_id_array=cond( is_set( $#collection_attributes[$attribute.id] ), $#collection_attributes[$attribute.id].content, $attribute.content )}

    {* Always set the .._selected_array_.. variable, this circumvents the problem when nothing is selected. *}
    <input type="hidden" name="{$attribute_base}_ezselect_selected_array_{$attribute.id}" value="" />

    <select class="{$html_class}" name="{$attribute_base}_ezselect_selected_array_{$attribute.id}[]" {if $attribute.class_content.is_multiselect}multiple="multiple"{/if}>
        {section var=Options loop=$attribute.class_content.options}
            <option value="{$Options.item.id}" {if $selected_id_array|contains( $Options.item.id )}selected="selected"{/if}>
                {if $is_valuation}{valuation_translation($Options.item.name)}{else}{$Options.item.name|wash( xhtml )}{/if}
            </option>
        {/section}
    </select>
{/let}
{undef $is_valuation}
{/default}
