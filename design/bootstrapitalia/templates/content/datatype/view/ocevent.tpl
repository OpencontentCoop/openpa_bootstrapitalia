<div class="point-list-wrapper my-4">
    ...
    {if count($events)|gt(10)}
        <p class="text-serif">{"L'evento si svolge"|i18n('bootstrapitalia')} {$attribute.content.text|wash()}</p>
        ...
        {if recurrences_strtotime($event.start)|datetime( 'custom', '%j%m%Y' )|ne(recurrences_strtotime($event.end)|datetime( 'custom', '%j%m%Y' ))}
            ...
            <div class="point-list">
                <div class="point-list-aside point-list-warning">
                    <div class="point-date text-monospace">{recurrences_strtotime($event.start)|datetime( 'custom', '%j' )}</div>
                    <div class="point-month text-monospace">{recurrences_strtotime($event.start)|datetime( 'custom', '%F' )}</div>
                </div>
            ...
            <div class="point-list">
                <div class="point-list-aside point-list-warning">
                    <div class="point-date text-monospace">{recurrences_strtotime($event.end)|datetime( 'custom', '%j' )}</div>
                    <div class="point-month text-monospace">{recurrences_strtotime($event.end)|datetime( 'custom', '%F' )}</div>
                </div>
                <div class="point-list-content">
                    <div class="card card-teaser shadow p-4 rounded border">
                        <div class="card-body">
