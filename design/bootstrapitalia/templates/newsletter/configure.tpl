    ...
    {* for every newsletter_system check if there are newsletter_list where a subscription for current siteaccess is possible *}
        ...
        {else}
                        ...
                        {if $list_node.data_map.newsletter_list.content.output_format_array|count()|ne(0)}
                            <td>
                                ...
                                <div class="form-check form-check-inline">              
                                  <input type="checkbox" id="list-{$list_id}" name="Subscription_ListArray[]" value="{$list_id}"{if and( $is_removed|not , is_set( $subscription_array[ $list_id ] ) )} checked="checked"{/if} title="{$list_node.data_map.title.content|wash}" />
                                ...
                                {if $list_node.data_map.newsletter_list.content.output_format_array|count()|gt(1)}
                                  ...
                                  {foreach $list_node.data_map.newsletter_list.content.output_format_array as $output_format_id => $output_format_name}
                                    <td class="newsletter-list">
                                      <div class="form-check form-check-inline"> 
                                        <input class="form-check-input" id="output-{$list_id}-{$output_format_id}" type="radio" name="Subscription_OutputFormatArray_{$list_id}[]" value="{$output_format_id|wash}" {if is_set( $list_selected_output_format_array[ $output_format_id ] )} checked="checked"{/if} title="{$output_format_name|wash}" /> 
                ...
                <div class="form-group row mb-3">
                  ...
                  <div class="col-sm-10">
                    {foreach $available_saluation_array as $salutation_id => $salutation_name}
                        <div class="form-check form-check-inline">
                          <input class="form-check-input" id="salutation-{$salutation_id|wash}"  type="radio" name="Subscription_Salutation" value="{$salutation_id}"{if $newsletter_user.salutation|eq($salutation_id)} checked="checked"{/if} title="{$salutation_name|wash}" />
                ...
                <div class="form-group row mb-3">
                  ...
                  <div class="col-sm-10">  
                    <input class="form-control" required="required" id="Subscription_Email" type="text" name="Subscription_Email" value="{$newsletter_user.email|wash}" title="{'Email of the subscriber.'|i18n( 'cjw_newsletter/configure' )}" {cond( is_set( $newsletter_user ), 'disabled="disabled"', '')} />
                  </div>
                </div>

                <div class="mb-3 clearfix">
                    <input class="btn btn-primary btn-lg pull-right" type="submit" name="ConfirmButton" value="{'Confirm'|i18n( 'cjw_newsletter/configure' )}" title="{'Add to subscription.'|i18n( 'cjw_newsletter/configure' )}" />
                </div>
            </form>
