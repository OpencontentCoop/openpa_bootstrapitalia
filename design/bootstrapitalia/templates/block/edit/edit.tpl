    ...
    <div class="block-content {if $action|eq( 'add' )}expanded{else}collapsed{/if}">
        ...
        <div class="block-controls float-break">
            <div class="left blockname">
                <label for="block-name-{$block_id}">{'Name:'|i18n( 'design/standard/block/edit' )}</label>
            ...
            <div class="right">
                {if $is_custom|not}
                    <label for="block-overflow-control-{$block_id}" class="d-none">{'Set overflow'|i18n( 'design/standard/block/edit' )}</label>
                    <select id="block-overflow-control-{$block_id}" class="d-none list block-control" name="ContentObjectAttribute_ezpage_block_overflow_{$attribute.id}[{$zone_id}][{$block_id}]">
                ...
                {/if}
                <label for="block-view-{$block_id}" class="d-inline">{'Block type:'|i18n( 'design/standard/datatype/ezpage' )}</label>
                <select id="block-view-{$block_id}" class="list block-control d-inline" name="ContentObjectAttribute_ezpage_block_view_{$attribute.id}[{$zone_id}][{$block_id}]">
        ...
        <div class="block-parameters float-break">
            ...
            {if $is_dynamic}
                ...
                <div class="block-parameter">
                    ...
                    <div class="source w-auto float-none font-weight-normal">
                ...
                {else}
                <div class="block-parameter">
                    <label for="block-fetch-parameter-{$fetch_parameter}-{$block_id}">{$fetch_parameter}:</label>
                    <input id="block-fetch-parameter-{$fetch_parameter}-{$block_id}" class="textfield block-control" type="text" name="ContentObjectAttribute_ezpage_block_fetch_param_{$attribute.id}[{$zone_id}][{$block_id}][{$fetch_parameter}]" value="{$fetch_params[$fetch_parameter]}" />
                </div>
                {/if}
                {/foreach}
            {/if}
            {if ezini_hasvariable( $block.type, 'CustomAttributes', 'block.ini' )}
                ...
                <div class="block-parameter">
                    ...
                    {if and( is_set( $use_browse_mode[$custom_attrib] ), eq( $use_browse_mode[$custom_attrib], 'true' ) )}
                        {if is_set($custom_attribute_names[$custom_attrib])}<label class="font-weight-bold w-100">{$custom_attribute_names[$custom_attrib]}:</label>{/if}
                    ...
                    {else}
                        {if is_set( $custom_attribute_types[$custom_attrib] )}
                            {switch match = $custom_attribute_types[$custom_attrib]}
                                {case match = 'text'}
                                    <label class="font-weight-bold w-100" for="block-custom_attribute-{$block_id}-{$loop_count}">{if is_set( $custom_attribute_names[$custom_attrib] )}{$custom_attribute_names[$custom_attrib]}{else}{$custom_attrib}{/if}:</label>
                ...
                <td id="z:{$zone_id}_b:{$block_id}_i:{$item.object_id}_h" class="handler">{$item_object.name|wash()}</td>
                    <td class="time">
                        ...
                        {else}
                            ...
                            <img class="schedule-handler" src="{'ezpage/clock_ico.gif'|ezimage(no)|shared_asset()}" alt="{concat( 'Publishing schedule for: '|i18n( 'design/standard/block/edit' ), $item_object.name|wash() )|shorten( '50' )}" title="{concat( 'Publishing schedule for: '|i18n( 'design/standard/block/edit' ), $item_object.name|wash() )|shorten( '50' )}" />
                        {/if}
                    </td>
            </tr>
            {undef $item_object}
        ...
        {if $use_styles}
            <div class="block-parameters float-break">
                <div class="block-parameter">
                    <label class="font-weight-bold w-100">Sfondo</label>
        ...
        <div class="block-parameters float-break">
            <div class="block-parameter">
                <label class="font-weight-bold w-100">Impostazioni di layout</label>
        ...
        {if and(ezini_hasvariable($block.type, 'CanAddShowAllLink', 'block.ini'), ezini($block.type, 'CanAddShowAllLink', 'block.ini')|eq('enabled') )}
            <div class="block-parameters float-break">
                <div class="block-parameter">
                    <label class="font-weight-bold w-100">Link all'origine</label>
        ...
        {if and(ezini_hasvariable($block.type, 'CanAddIntroText', 'block.ini'), ezini($block.type, 'CanAddIntroText', 'block.ini')|eq('enabled') )}
            <div class="block-parameters float-break">
                <div class="block-parameter">
                    <label class="font-weight-bold w-100">Testo introduttivo</label>
