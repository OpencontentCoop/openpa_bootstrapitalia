<div class="mb-5">
    {if $post.object.can_edit}
        <form method="post" action="{"content/action"|ezurl(no)}" style="display: inline;">
            ...
            <button class="btn btn-info rounded-0 text-white" type="submit" name="EditButton">
                <i aria-hidden="true" class="fa fa-pencil mr-2 me-2"></i> {'Edit'|i18n('editorialstuff/dashboard')}
            </button>
    ...
    {if $post.object.can_remove}
        <form method="post" action="{"content/action"|ezurl(no)}" style="display: inline;">
            ...
            <button class="btn btn-info rounded-0  text-white" type="submit" name="ActionRemove">
                <i aria-hidden="true" class="fa fa-trash mr-2 me-2"></i> {'Delete'|i18n('design/standard/gui')}
            </button>
    ...
    <a class="btn btn-info rounded-0 text-white" href="{$post.node.url_alias|ezurl(no)}">
        <i aria-hidden="true" class="fa fa-eye mr-2 me-2"></i> {'View on website'|i18n('editorialstuff/dashboard')}
    </a>
    ...
    <div class="btn-group pull-right">
        <button type="button" class="py-2 btn btn-link rounded-0 text-black dropdown-toggle text-decoration-none" data-toggle="dropdown" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        ...
                        {foreach $assignable_states as $key => $state}
                            <li>
                                <a class="list-item" href="{concat('editorialstuff/state_assign/', $factory_identifier, '/', $key, "/", $post.object.id )|ezurl(no)}">
                                    <span><div class="d-inline-block mr-1 me-1 ml-1 ms-1 rounded label-{$state.identifier|wash()}" style="vertical-align:middle; width: 13px;height: 13px"></div> {$state.current_translation.name|wash}</span>
                                </a>
                            </li>
                        {/foreach}
                    </ul>
