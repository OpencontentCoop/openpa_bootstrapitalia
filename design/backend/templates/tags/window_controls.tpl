...
<ul class="tabs{if $read_open_tab_by_cookie} tabs-by-cookie{/if}">
    ...
    {if and( $tag_exists, $tag.main_tag_id|eq( 0 ) )}
        ...
        <li id="node-tab-tags-description" class="middle{if $tab_index|eq('description')} selected{/if}">
            <a href={concat( $tag_url, '/(tab)/description' )|ezurl}>{'Description'|i18n( 'bootstrapitalia/tag_description' )}</a>
        </li>
    {/if}
