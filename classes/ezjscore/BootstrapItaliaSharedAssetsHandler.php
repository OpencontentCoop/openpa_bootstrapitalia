...
class BootstrapItaliaSharedAssetsHandler implements ezjscSharedHandler
{
    ...
    public function __construct()
    {
        ...
        $this->prefix = trim($this->settings['Prefix'], '/');
    }
    ...
    public function packFiles($data, $subPath)
    {
        ...
        if (!$this->cachedFileExists($data['cache_path'])) {
            ...
            $dataList = $this->compileAssetsList($data);
            $this->storeCachedFile($data['cache_path'] . '.json', json_encode($dataList), false);
            $this->storeCachedFile($data['cache_path'], $content, $data['file_extension'] === '.css');
        }

        $urls = [];
        foreach ($data['http'] as $file) {
            if (!empty($file)) {
                $urls[] = $file;
            }
        }
        $urls[] = $this->applyServerUrl($data['cache_path']);
    ...
    private function compileAssetsList($data)
    {
        $list = [];
        foreach ($data['http'] as $file) {
            if (!empty($file)) {
