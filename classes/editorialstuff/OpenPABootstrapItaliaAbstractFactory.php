...
abstract class OpenPABootstrapItaliaAbstractFactory extends OCEditorialStuffPostDefaultFactory
{
    ...
    public function dashboardModuleResult($parameters, OCEditorialStuffHandlerInterface $handler, eZModule $module)
    {
        ...
        $Result['path'] = array(
            array(
                ...
                'text' => ezpI18n::tr( 'bootstrapitalia', "Dashboard")
            ),
            array(
                'url' => false,
                'text' => isset($this->configuration['Name']) ? $this->configuration['Name'] : $this->configuration['ClassIdentifier']
            )
        );
        return $Result;
    }
    ...
    public function editModuleResult($parameters, OCEditorialStuffHandlerInterface $handler, eZModule $module)
    {
        ...
        $Result['path'] = array(
            ...
            array(
                'url' => 'editorialstuff/dashboard/' . $this->configuration['identifier'],
                'text' => isset($this->configuration['Name']) ? $this->configuration['Name'] : $this->configuration['ClassIdentifier']
            )
        );
        if ($currentPost instanceof OCEditorialStuffPostInterface) {
