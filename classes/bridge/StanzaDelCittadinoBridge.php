...
//            throw new ServiceToolsException('Local service channel not found');
    ...
    public function checkServiceSync(eZContentObjectTreeNode $service, StanzaDelCittadinoClient $client = null, $domain = null): array
    {
        $serviceSync = new ServiceSync($service, $client ?? $this->instanceNewClient(), $domain);
        return [
            'info' => $serviceSync->getInfo(),
            'errors' => $serviceSync->getErrors(),
        ];
    }
}